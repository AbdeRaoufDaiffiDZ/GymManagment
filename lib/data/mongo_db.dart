...
class MongoDatabase {
  ...
  Future<Either<Failure, List<User_Data>>> RetriveData(
    ...
    try {
      ...
      usersList = users.map((doc) => User_Data.fromMap(doc)).toList();

      for (User_Data user in usersList) {
        int lastEnter =
  ...
  int calculateMonthsDifference(String? lastCheckDate, DateTime? end) {
    DateTime? lastDate = null;
    if (lastCheckDate == null || lastCheckDate.isEmpty) {
      lastDate = end;
    } else {
// Parse the date string (assuming it's in "yyyy-MM-dd" format)
      lastDate = DateTime.tryParse(lastCheckDate);
    }
    ...
    int yearDiff = today.year - lastDate.year;
    int monthDiff = today.month - lastDate.month;
...
/////////////////////////////////////////////////////////////////////////////////////////// product data fucntions
  ...
  bool checkif3daysAgo(DateTime date) {
    DateTime today = DateTime.now(); // Current date
    DateTime threeDaysAgo =
        today.subtract(Duration(days: 3)); // Get the date 3 days ago
  ...
  Future<Either<Failure, List<Product>>> RetriveProducts(
    ...
    try {
      ...
      List<Product> products =
          product.map((doc) => Product.fromMap(doc)).toList();
      for (var pr in products) {
        if (pr.saleRecords.isNotEmpty) {
          for (var rc in pr.saleRecords) {
            bool isOld = checkif3daysAgo(rc.dateTime);

            if (isOld) {
              pr.saleRecords.remove(rc);
